...
function checkReachIntoPackage({
  context,
  namespace,
  importRe,
  node,
  ...
  context.report({
    node,
    message: `cannot reach into package (${importName}), use (${should})`
  });
  return false;
}
...
function checkCircularImport({
  ...
  if (results) {
    ...
    context.report({
      node,
      message: `when importing to same package (${importName}), use relative imports (${suggest})`
    });
    return false;
  }
...
module.exports = {
  ...
  create: context => {
    ...
    const checkPackage = (node, importName) =>
      checkReachIntoPackage({
        node,
        importName,
        context,
        namespace,
